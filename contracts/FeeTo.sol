...
contract FeeTo {
    ...
    function renounce(address pair) public returns (uint value) {
        ...
        TokenAllowState storage token0AllowState = tokenAllowStates[IBeefswapV2Pair(pair).token0()];
        TokenAllowState storage token1AllowState = tokenAllowStates[IBeefswapV2Pair(pair).token1()];
        ...
        if (
            ...
            value = IBeefswapV2Pair(pair).balanceOf(address(this));
            if (value > 0) {
                ...
                assert(IBeefswapV2Pair(pair).transfer(pair, value));
                IBeefswapV2Pair(pair).burn(pair);
            }
    ...
    function claim(address pair) public returns (uint value) {
        ...
        if (
            ...
            if (value > 0) {
                ...
                assert(IBeefswapV2Pair(pair).transfer(feeRecipient, value));
            }
        }
    }
}

interface IBeefswapV2Pair {
