...
import BeefswapV2Factory from '@beefswap/core/build/BeefswapV2Factory.json'
import BeefswapV2Pair from '@beefswap/core/build/BeefswapV2Pair.json'
...
describe('scenario:FeeTo', () => {
  ...
  beforeEach('deploy beefswap v2', async () => {
    factory = await deployContract(wallet, BeefswapV2Factory, [wallet.address])
  })
  ...
  describe('tokens', () => {
    ...
    beforeEach('make test tokens', async () => {
      const { timestamp: now } = await provider.getBlock('latest')
      const token0 = await deployContract(wallet, Beef, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token0)
      const token1 = await deployContract(wallet, Beef, [wallet.address, constants.AddressZero, now + 60 * 60])
      tokens.push(token1)
    })
    ...
    beforeEach('create fee liquidity', async () => {
      ...
      pair = new Contract(pairAddress, BeefswapV2Pair.abi).connect(wallet)
